...
func TestBracket(t *testing.T) {
	...
	expectedEncodedMarkup := ""
	assert.Equal(t, expectedEncodedMarkup, string(responseData))
}
...
func TestBracketWithDate(t *testing.T) {
	url := "https://liquipedia.net/rocketleague/Rocket_League_Championship_Series/2025/Raleigh_Major/North_America/Open_6"
	encodedUrl := utils.EncodedBase64(url)
	endpoint := fmt.Sprintf("/bracket/%s/date/25", encodedUrl)
...
func TestMakeThreadWithDate(t *testing.T) {
	url := "https://liquipedia.net/rocketleague/Rocket_League_Championship_Series/2022-23/Winter/Asia-Pacific/Cup"
	encodedUrl := utils.EncodedBase64(url)
	template := utils.EncodedBase64("groups-bracketrd1-streams")
	endpoint := fmt.Sprintf("/makethread/%s/template/%s/date/11", encodedUrl, template)
...
func TestPrizepool1v1(t *testing.T) {
	...
	expectedEncodedMarkup := ""
	assert.Equal(t, expectedEncodedMarkup, string(responseData))
}
...
func TestScheduleWithDate(t *testing.T) {
	url := "https://liquipedia.net/rocketleague/Rocket_League_Championship_Series/2022-23/Fall/North_America/Cup"
	encodedUrl := utils.EncodedBase64(url)

	// Day 1
	endpoint := fmt.Sprintf("/schedule/%s/date/21", encodedUrl)
	responseData := responseForEndpoint(t, endpoint)
	...
	// Day 3
	endpoint = fmt.Sprintf("/schedule/%s/date/23", encodedUrl)
	responseData = responseForEndpoint(t, endpoint)
